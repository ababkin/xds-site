...
import Types (Source(..), NewSource(..), Dataset(..), NewSourceUrl(..))
import Store.DDB.Types ()
import Store.DDB (runWithCreds)
...
createSource
  :: NewSource
  -> IO ()
createSource NewSource{nsUserId, nsTitle, nsDescription,
    nsUrls=NewSourceUrl{nsuUuid, nsuSourceUrl, nsuDatasetUrl} } = do
